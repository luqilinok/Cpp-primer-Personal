...
{
	Sales_data ret(lhs);  //把lsh的内容拷贝到临时变量ret中，这种做法便于运算
	ret += rhs;  //把rhs的内容加入其中
	return ret;  //返回ret
}
...
int main()
{
	...
	while (std::cin>>book)
	{
		std::cout << "ISBN、售出本数、原始价格、实际价格、折扣为" << book << std::endl;
	}
	...
	if (std::cin>>trans1)
	{
		while (std::cin>>trans1)
		{
			...
			{
				std::cout << trans.isbn() << "共有" << num << "条销售记录" << std::endl;
			}
		}
	}
	else
