...
这个循环将elem开始的内存中的string对象移动到dest开始的内存中，每个循环步骤中，调用construct在新内存空间中创建对象，若第二个参数是一个左值，则进行拷贝动作。在上面的代码中，用std::move将一个左值转换为右值引用，这样，construct会调用string的移动构造函数将数据从就内存空间移动而不是拷贝到内存空间中，避免了不必要的操作

ex16_49

g(42)匹配第3个模板，T被推断为int
g(p)匹配第4个模板，T被推断为int
g(ci)匹配第3个模板，T被推断为int
g(p2)匹配第4个模板，T被推断为const int
f(42)匹配第一个模板，T被推断为int
f(p)匹配第一个模板，T被推断为int*
f(ci)匹配第一个模板，T被推断为int
f(p2)匹配第2个模板，T被推断为int
...
print需要函数类型支持<<运算符，所以会产生编译错误

ex16_55

如果非可变参数版本放在可变参数版本之后，那么就会产生“当定义可变参数版本的print时，非可变参数版本的声明必须在作用域中，否则，可变版本参数会无限递归”的情况
...
重载会影响函数匹配，而特例化不会影响函数匹配

ex16_67

定义特例化版本不会影响debug_rep的函数匹配，因为特例化不得本质是实例化一个函数模板，而非重载它，因此，特例化不会影响函数匹配
